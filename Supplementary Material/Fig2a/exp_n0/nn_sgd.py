...
with experiment.train():
    for index_n1 in range(len(args.inputSize)):
        for index_n3 in range(len(args.outputSize)):
            ...
            x = torch.tensor(np.genfromtxt("x_in_{}_100_mod_n0.csv".format(it_ind), delimiter=','), device=device).float()
            y = torch.tensor(np.genfromtxt("y_out_{}_100_mod_n0.csv".format(it_ind), delimiter=','), device=device).float()
